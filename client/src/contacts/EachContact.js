...
export default function EachContact({ contact, fetchData }) {
    const [nameValue, setNameValue] = useState(contact.Name);
    const [contactNumberValue, setContactNumberValue] = useState(contact.MobileNumber);
    const [emailValue, setEmailValue] = useState(contact.Email);
    const [cityValue, setCityValue] = useState(contact.City);
    const [countryValue, setCountryValue] = useState(contact.Country);

    const openModal = () => {
        document.getElementById('new-modal-' + contact.ID).classList.remove("hidden");
    }
    const closeModal = () => {
        document.getElementById('new-modal-' + contact.ID).classList.add("hidden");
    }
    const completeForm = () => {
        closeModal()
        fetchData()
    ...
    const deleteContact = () => {
        if (window.confirm("Are you sure you want to delete this contact??") == true) {
            axios.delete(`${API_URL}/contacts/${contact.ID}`)
                .then(res => fetchData())
                .catch(error => console.log(error.response))
        } else {
        ...
        <div className="bg-slate-100 rounded-lg mb-4 p-4 hover:border hover:border-purple-700">
            <div>
                <div>
                    ...
                    {/* <div className="text-slate-400">{contact.MobileNumber}</div>
                    <div className="text-slate-400">{contact.Email}</div> */}
                </div>
                <div className="text-sm flex space-x-4 mt-4">
                    <Link to={`/profile/${contact.ID}`}>View Profile</Link>
                    <button onClick={openModal} >Edit</button>
                    <button onClick={deleteContact} className="text-red-600">Delete</button>
                </div>
            </div>

            {/* Start Modal */}
            <div className="relative z-10 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" id={`new-modal-${contact.ID}`}>
                ...
                <div className="fixed z-10 inset-0 overflow-y-auto">
                    <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div className="relative inline-block align-middle bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg w-full">
                            <form id={`editform-${contact.ID}`} onSubmit={updateContact} method="post">
                                <div className="bg-white">
                                    <div className="flex justify-between px-8 py-4 border-b">
                                        <h1 className="font-medium">Update Contact</h1>
                                    ...
                                    <div className="px-8 py-8">
                                        ...
                                        <div className="mb-5">
                                            ...
                                            <input type="text" name="MobileNumber" value={contactNumberValue} onChange={(e) => setContactNumberValue(e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
                                        </div>
                                        <div className="mb-5">
